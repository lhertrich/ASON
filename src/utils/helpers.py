...
def get_image_and_mask_paths(data_dir, mask_dir):
    image_paths_train = sorted([data_dir + "/" + path for path in os.listdir(data_dir) if not path.startswith(".")])
    mask_paths_train = sorted([mask_dir + "/" + path for path in os.listdir(mask_dir) if not path.startswith(".")])
    image_paths_test = sorted([data_dir + "_test/" + path for path in os.listdir(data_dir+"_test") if not path.startswith(".")])
    mask_paths_test = sorted([mask_dir + "_test/" + path for path in os.listdir(mask_dir+"_test") if not path.startswith(".")])
